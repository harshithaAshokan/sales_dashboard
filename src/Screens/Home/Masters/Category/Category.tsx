import { useEffect, useState } from "react";
import { useDispatch} from "react-redux";
import classes from "./Category.module.css";
import { Space, Table, Layout, Typography, Pagination, Tooltip, Row, Col } from "antd";
import deleteicon from "../../../../assets/delete.png";
import updateicon from "../../../../assets/system-update.png";
import filtericon from "../../../../assets/setting.png";
import {
  handleUserId,
} from "../../../../redux/reducers/AuthReducers";
import { listCategory } from "../../../../axios/services";
import MastersAddModal from "../../Modal/MastersModal/MastersAddModal";
import MastersDeleteModal from "../../Modal/MastersModal/MastersDeleteModal";
import MastersFilter from "../../Modal/MastersModal/MastersFilter";
import { useToken } from "../../../../utility/hooks";
import { masterFilterProps } from "../../../../type/filter";
import { categoryListProps } from "../../../../type/masters";
import { Helmet } from "react-helmet";
const { Content } = Layout;
const { Title } = Typography;

export default function Category() {
  const dispatch = useDispatch();
  const token = useToken();
  const [currentPage, setCurrentPage] = useState(1);
  const [totalItems, setTotalItems] = useState(0);
  const [filter, setFilter] = useState(false);
  const [update, setUpdate] = useState(false);
  const [add, setAdd] = useState(false);
  const [deleteModal, setDeleteModal] = useState(false);
  const [categoryList,setCategoryList] = useState<categoryListProps[]>([])
  const filterValue = {
    name:""
  }
  const handleAdd = () => {
    setAdd(true);
  };

  const handleDelete = (userId:string) => {
    setDeleteModal(true);
    dispatch(handleUserId(userId));
  };

  const handleClose = () => {
    setDeleteModal(false);
    setAdd(false);
    setUpdate(false);
  };

  const handleUpdate = (userId:string) => {
    setUpdate(true);
    dispatch(handleUserId(userId));
  };

  const handlePageChange = (page:number) => {
    setCurrentPage(page);
  };

  const handleCategoryData = (page = 1, pageSize = 10, values:masterFilterProps) => {
    const formData = new FormData();
    formData.append("token", token);
    if (values?.name) {
      formData.append("name", values.name);
    }
    listCategory(page, pageSize, formData)
      .then((response) => {
        setCategoryList(response.data.data.items);
        setTotalItems(response.data.data.total_count);
      })
      .catch((error) => {
        console.log(error);
      });
  };

  useEffect(() => {
    if (token) {
      handleCategoryData(currentPage,10,filterValue);
    }
  }, [token, currentPage]);

  const columns = [
    {
      title: "S.No",
      dataIndex: "index",
      render: (text:string, record:categoryListProps, index:number) => index + 1 + (currentPage - 1) * 10,
    },
    {
      title: "Name",
      dataIndex: "customerCategoryName",
      key: "customerCategoryName",
    },

    {
      title: "Action",
      key: "action",
      render: (_:string, record:categoryListProps) => (
        <Space size="middle">
          <Tooltip placement="bottom" title="Update">
            <img
              src={updateicon}
              width="25px"
              height="25px"
              onClick={() => handleUpdate(record.customerCategoryId)}
            ></img>
          </Tooltip>
          <Tooltip placement="bottom" title="Delete">
            <img
              src={deleteicon}
              width="25px"
              height="25px"
              onClick={() => handleDelete(record.customerCategoryId)}
            ></img>
          </Tooltip>
        </Space>
      ),
    },
  ];
  return (
    <>
     <Helmet>
      <title>Category</title>
     </Helmet>
    
     <Layout className="px-5">
    <Content className={`px-3 mt-4 ${classes.content}`}>
          <Row className="mt-4">
            <Col span={12}><Title level={2} className="fw-bold float-start ms-3">
            Category Management
          </Title>
          </Col>
          <Col span={10}>
            <button className="btn button fs-6 float-end mt-2 " onClick={handleAdd}>
            Add Category
          </button>
          </Col>
          <Col span={2}>
          <img
            src={filtericon}
            width="25px"
            height="25px"
            className="float-start mt-3 ms-5"
            onClick={() => setFilter(!filter)}
          ></img>
          </Col>
        </Row>
        <div className="ms-5">
          {filter && <MastersFilter listapical={handleCategoryData}/>}
        </div>

        <Table
          columns={columns}
          dataSource={categoryList}
          pagination={false}
         
          className={`mt-2 table-responsive mx-auto py-3 ${classes.table}`}
            scroll={{ x: "max-content" }}
        />
        {add && (
          <MastersAddModal
            listapical={handleCategoryData}
            value="category"
            add={add}
            update={update}
            close={handleClose}
            list={categoryList}
            current={currentPage}
          />
        )}
        {deleteModal && (
          <MastersDeleteModal
            listapical={handleCategoryData}
            value="customer_category"
            close={handleClose}
            current={currentPage}
          />
        )}
        {update && (
          <MastersAddModal
            listapical={handleCategoryData}
            value="category"
            update={update}
            add={add}
            close={handleClose}
            list={categoryList}
            current={currentPage}
          />
        )}
        <Pagination
          current={currentPage}
          pageSize={10}
          total={totalItems}
          onChange={handlePageChange}
        />
      </Content>
    </Layout>
    </>
  );
}
